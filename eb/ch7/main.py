...
# The two merging algorithms:
# Single linkage is just taking the minimum of two arguments.
# UPGMA will take the average of the two arguments.
single_linkage = min
upgma = lambda a, b: (a + b) / 2

# This WeightedTreeNode class represents the tree that generates the Newick format.
# It is built using a bottom-up strategy in the merge_clusters function. This
# class handles holding the nodes, branch organization, weights (i.e. branch lengths)
# and printing out the Newick format itself.
class WeightedTreeNode:
    ...
    def _tostr(self):
        """For internal use. Recursively create a Newick format representation of the node. Does not include the terminal semicolon."""
        if not self.left and not self.right:
            return self.name
        return "(" + self.left._tostr() + ":" + "{:.5f}".format(self.lweight) + "," + self.right._tostr() + ":" + "{:.5f}".format(self.rweight) + ")"

    def __str__(self):
        """Generate the Newick format representation of this node."""
        return self._tostr() + ";"

    def cumulative_weight(self):
        """Gets the total height of this node and its children."""
        if not self.left:
            return 0
        return self.lweight + self.left.cumulative_weight()

    def merge(n1, n2, distance):
        """Merge two nodes by creating a parent node and splitting the distance into two equally-lengthed branches.

        Keyword arguments:
        n1 -- The (left) node for the first sequence
        n2 -- The (right) node for the second sequence
        distance -- The distance between the two sequences
        """
        parent = WeightedTreeNode(str(n1) + str(n2))
        parent.left = n1
        parent.right = n2
        half_distance = distance / 2
        parent.lweight = half_distance - n1.cumulative_weight()
        parent.rweight = half_distance - n2.cumulative_weight()
        return parent

def merge_clusters(i, j, clusters, tree, merger=single_linkage):
    """Merge two clusters in from a distance matrix and build the Newick tree.

    Keyword arguments:
    i -- The index of the first cluster to merge
    j -- The index of the second cluster to merge
    clusters -- The nested dictionary representing the distance matrix
    merger -- The merging algorithm to use. Defaults to single_linkage.
    """
    # Determine all the names
    cluster_names = list(clusters.keys())
    a, b = cluster_names[i], cluster_names[j]
    new_cluster_name = a + b

    del cluster_names[i]
    del cluster_names[cluster_names.index(b)]

    # Find the distance for this pair of clusters for building the tree.
    distance = clusters[a][b]

    # For all clusters except a and b (which are already removed),
    # we will set (x, ab) to min((x, a), (x, b))
    for cluster in cluster_names:
        subcluster = clusters[cluster]
        # Apply the merging algorithm to the two subcluster values
        smallest_distance = merger(subcluster[a], subcluster[b])
        del subcluster[a], subcluster[b]

        # Store the new distance value for the merged cluster
        # If this is the first iteration, it will initialize the
        # nested dictionary to begin with.
        if new_cluster_name in clusters:
            clusters[new_cluster_name][cluster] = smallest_distance
        else:
            clusters[new_cluster_name] = {cluster: smallest_distance}
        # Modify the distance values for the mirrored side of the matrix
        if cluster in clusters:
            clusters[cluster][new_cluster_name] = smallest_distance
        else:
            clusters[cluster] = {new_cluster_name: smallest_distance}
    ...
    # Also add in the cluster's own distance
    clusters[new_cluster_name][new_cluster_name] = 0.0

    # Merge the two clusters in the Newick tree
    node1 = tree[a]
    del tree[a]
    node2 = tree[b]
    del tree[b]
    parent = WeightedTreeNode.merge(node1, node2, distance)
    tree[new_cluster_name] = parent

def generate_newick_tree(clusters):
    # Start the cluster merging process
    num_clusters = len(clusters)
    tree = { name: WeightedTreeNode(name) for name in clusters.keys() }

    # Keep going until we only have one cluster pair remaining
    while num_clusters > 2:
        # Find the minimum value in the distance matrix
        cluster_names = list(clusters.keys())
        best_distance = -1
        best_i = 0
        best_j = 0
        for i in range(num_clusters):
            for j in range(num_clusters):
                distance = clusters[cluster_names[i]][cluster_names[j]]
                if distance > 0 and (best_distance == -1 or distance < best_distance):
                    best_distance, best_i, best_j = distance, i, j
        i, j = best_i, best_j
        num_clusters -= 1

        # Perform the merging process for the two clusters
        merge_clusters(i, j, clusters, tree, merger=single_linkage)

    # Merge the last cluster pair
    name1, name2 = list(tree.keys())
    node1, node2 = tree[name1], tree[name2]
    root = WeightedTreeNode.merge(node1, node2, clusters[name1][name2])
...
# Grab the distance matrix file from the user
filename = input('Phylip distances file [distance_matrix.txt]: ').strip()
if not filename:
    filename = "distance_matrix.txt"

# Grab the merging algorithm from the user
merger = upgma
merger_str = input("Merger [UPGMA]: ").strip().lower()
if merger_str == "single linkage" or merger_str == "single":
    merger = single_linkage
...
num_seq = int(lines[0].strip())
lines = lines[1:]

# Decipher the distance matrix
cluster_names = []
distances = []
for line in lines:
    columns = re.split(r"\s+", line)
    cluster_names.append(columns[0])
    distances.append(list(map(lambda x: float(x) if x else 0.0, columns[1:])))

# Convert the parallel list structure to a nested dictionary
clusters = {}
for i in range(num_seq):
    for j in range(num_seq):
        if cluster_names[i] in clusters:
            clusters[cluster_names[i]][cluster_names[j]] = distances[i][j]
        else:
            clusters[cluster_names[i]] = { cluster_names[j]: distances[i][j] }

# Create the Newick tree and print the output
tree = generate_newick_tree(clusters)
