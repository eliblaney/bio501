...
def align_sequences(alignment, find_all=True, align1='', align2='', cur_row=None, cur_col=None):
    ...
    align_type, alignments, gap_score, matrix, name1, name2, seq1, seq2, substitution_matrix = [v[1] for v in sorted(alignment.items())]
    alignments = []
...
def print_alignment(alignment, substitution_matrix_name=None, num=1, output_buffer=print):
...
def align_multiple(seqs, smatrix, gap_score=-1, align_type='global'):
...
def print_multiple_alignments(alignments, substitution_matrix_name=None, output_buffer=print, line_wrap=60, prefix_len=16):
...
smatrix = input('Substitution Matrix [PAM250]: ').lower()
if smatrix:
    smatrix += ".txt"
else:
    smatrix = "pam250.txt"
